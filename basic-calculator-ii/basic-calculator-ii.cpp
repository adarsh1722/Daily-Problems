...
public:
    ...
    int calculate(string s) {
          ...
          // Approach : converting first into postfix then evaluating
          // space    : O(m + n + p);
          // time     : O(N)
        ...
        for(int i = 0 ; i< postfix.size()  ;i++)
        {
            ...
            int op2 =  st2.top();st2.pop();
            int op1 = st2.top() ;st2.pop();
            if(postfix[i] == "+") st2.push(op1+op2);
            else if(postfix[i] == "-") st2.push(op1-op2);
            else if(postfix[i] == "*") st2.push(op1*op2);
            else st2.push(op1/op2);
